...
// Alamat sensor BMP280
#define ADDR_1       0x76
#define ADDR_2       0x77

// Alamat Arduino kedua sebagai I2C slave
#define SLAVE_ARDUINO_ADDR 0x08
...
float readTemperature(uint8_t addr, BMP280_Calib &c, float offset) {
  ...
  int32_t adc = ((uint32_t)Wire.read() << 12) |
...
float readPressure(uint8_t addr, BMP280_Calib &c) {
  ...
  return (float)(((p + v1 + v2) >> 8) +
                 ((int64_t)c.P7 << 4)) / 256.0f;
}
...
float updateKalman(KalmanFilter &kf, float z) {
  ...
  float K = kf.error_estimate /
            (kf.error_estimate + kf.error_measure);
...
bool checkSensor(uint8_t addr) {
  ...
  return (id == 0x58 || id == 0x60);
}

// ── Kirim data ke Arduino kedua via I2C ───────────────────────────────────────

void sendDataToSlave(float dP, float raw_v, float filtered_v) {
  Wire.beginTransmission(SLAVE_ARDUINO_ADDR);

  Wire.write((byte*)&dP, sizeof(dP));
  Wire.write((byte*)&raw_v, sizeof(raw_v));
  Wire.write((byte*)&filtered_v, sizeof(filtered_v));
...
void setup() {
  ...
  if (checkSensor(ADDR_1)) {
    Serial.println(F("OK"));
    initSensor(ADDR_1, cal1);
  } else {
    Serial.println(F("TIDAK TERDETEKSI - cek wiring!"));
  }

  Serial.print(F("Sensor 0x77 (static): "));
  if (checkSensor(ADDR_2)) {
    Serial.println(F("OK"));
    initSensor(ADDR_2, cal2);
  } else {
    Serial.println(F("TIDAK TERDETEKSI - cek wiring!"));
  }
  ...
  Serial.println(pressure_offset, 3);
...
void loop() {
  ...
  sendDataToSlave(dP, raw_v, filtered_v);
